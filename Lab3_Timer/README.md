...
* **理解非同步中斷機制**：觀察核心如何在不影響系統運作的前提下，由硬體時鐘觸發預約的函數，即使沒有使用者輸入，核心也能根據時間觸發特定行為。

## 成果展示
![實驗三成果圖](images/Lab3_result.png)

## 挑戰與心得
* 震撼實驗：見證內部中斷 (Internel Interrupt) 與 Kernel Panic： 在實驗過程中，筆者主動測試了「不刪除定時器移除模組」的情況。當核心試圖跳轉至已釋放的記憶體位址執行 Callback 時，立即觸發了內部異常導致系統崩潰。這次經驗也深刻理解到 del_timer 不僅是釋放資源，更是維護系統穩定性的關鍵防線。

* 非同步預約機制 (Asynchronous Scheduling)： 這是筆者第一次體會到核心如何處理「預約任務」。不同於 User-space 使用 sleep() 會阻塞進程，核心定時器像是在硬體層級掛了個鬧鐘，時間到後由 外部中斷 (Hardware Interrupt) 觸發，這種「不佔用 CPU 等待」的非同步特性，是高效率系統的核心。
