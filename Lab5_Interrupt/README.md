...
* 中斷上下文的極限約束 (ISR Constraint)：撰寫 request_irq 的回呼函數時，必須隨時警惕「不可睡眠」的原則。雖然目前僅使用 printk，但這讓筆者理解到，要在極短時間內回應硬體需求，程式碼的效率與原子性（Atomicity）是首要考量。

* 硬體 Debug 的層級化思考：從檢查 Pin 9 的接地（Grounding）是否正確，到驗證按鈕在麵包板上的導通方向。這次實驗讓筆者補足了從「虛擬程式碼」到「實體電子流」之間的觀念。
